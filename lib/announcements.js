...
function formatCaption(gameData, localized) {
    return [
        ...
        `<s>${localized.org_price_usd ?? '{price_usd}'}/${localized.org_price_eur ?? '{price_eur}'}</s> <b>${localized.free ?? '{free}'}</b> ${localized.until ?? '{until}'} • ${ [localized.platform, ...localized.flags].join(' • ') }`,
