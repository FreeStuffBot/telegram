...
async function announceGame(gameData, type) {
    ...
    for (const channelLanguage in channels) {
        ...
        try {
            ...
            console.log('Failed to announce to', channelId, error);
        }
    }
}

async function fetchAndAnnounce(gameIds, testing = false) {
    console.log('Fetching and announcing games:', gameIds);
    ...
    for (const gameId of gamesIds) {
        ...
        const type = testing ? 'test' : 'non_trash'; //TODO: Process the game's flags.
...
async function handleFreeStuffAPIEvent(event) {
    if (event.event === 'free_games') {
        ...
        const [a, b, c, d, e] = await api.getGameList('all');
        await fetchAndAnnounce([a, b, c, d, e], true);
    }
}
